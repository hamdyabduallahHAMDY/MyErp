namespace MyErp.Core.Models;

public class Product : Common
{
    public string InternalId { get; set; }
    public string Name { get; set; }
    public string? statment { get; set; }
    public string? statment2 { get; set; }
    public string? UnitCode { get; set; }
    public string? Barcode { get; set; }
    public string TypeId { get; set; }
    public int CategoryId { get; set; }  // int, nullable if optional
    public Category Category { get; set; }
    public decimal? purchaseprice { get; set; }
    public decimal? sellingprice1 { get; set; }
    public decimal? sellingprice2 { get; set; }
    public decimal? sellingprice3 { get; set; }
    public decimal? sellingprice4 { get; set; }
    public decimal? sellingprice5 { get; set; }
    public decimal? sellingprice6 { get; set; }
    public decimal? sellingprice7 { get; set; }
    public decimal? sellingprice8 { get; set; }
    public string? serialnumber { get; set; }
    public decimal? sellingprice9 { get; set; }
    public decimal? sellingprice10 { get; set; }
    public decimal? discount { get; set; }
    public decimal? discount1 { get; set; }
    public Customer Customer { get; set; }
    public int? CustomerId { get; set; }
    public decimal? discount2 { get; set; }
    public decimal? discount3 { get; set; }
    public decimal? discount4 { get; set; }
    public int? limit { get; set; }
    public decimal? Tax { get; set; }
    public string? DescaraptionEnglish { get; set; }
    public Boolean? status { get; set; }
    public string? image { get; set; }
    public int? StockId { get; set; }
    public int? profitpercatanage { get; set; }
    public DateTime? created { get; set; }
    public DateTime? expire { get; set; }
    public decimal? TableTax { get; set; }
    public decimal? wholesaleprice { get; set; }
    public decimal? HALFwholesaleprice { get; set; }
    public decimal? Sectoral { get; set; }
}

